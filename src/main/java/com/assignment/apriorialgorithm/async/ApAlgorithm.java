package com.assignment.apriorialgorithm.async;
...
//	@OutputTimeUnit(TimeUnit.SECONDS)
	 ...
	 *
	 * args configuration parameters: args[0] is a filename, args[1] the min
...
//		}
	 ...
	 * the new ones
	 */
	private void createNewItemsetsFromPreviousOnes() throws ExecutionException, InterruptedException {
		// by construction, all existing itemsets have the same size
		int currentSizeOfItemsets = currentItemsets.get(0).length;
		Map<String, int[]> tempCandidates = new HashMap<>(); // temporary candidates
		// using async
		List<Future<Map<String,int[]>>> results = new ArrayList<>();
		for(int i = 0; i < numOfProcessors; i++) {
			int[] startEnd = calculateStartEndIndex(numOfProcessors,i,currentItemsets.size());
			// comparison can be done parallelly.
			// at the end, putall hashmap futures into tempCandidates.
			Future<Map<String,int[]>> result = executors.submit(new
					GenerateNewItemsetsTask(currentItemsets,startEnd[0],startEnd[1]));
			results.add(result);
		}
		...
		for(Future<Map<String,int[]>> result : results) {
			tempCandidates.putAll(result.get());
		}
		currentItemsets = new ArrayList<>(tempCandidates.values());
...
class GenerateNewItemsetsTask implements Callable<Map<String, int[]>> {
	List<int[]> currentItemsets;
	int start;
	int end;

	@Override
	public Map<String, int[]> call() {
		int currentSizeOfItemsets = currentItemsets.get(0).length;
		Map<String, int[]> partialCandidates = new HashMap<>();
		for(int i = start; i <= end; i++) {
			for (int j = i + 1; j < currentItemsets.size(); j++) {
				...
				if (ndifferent == 1) {
					...
					Arrays.sort(newCand);
					partialCandidates.put(Arrays.toString(newCand), newCand);
				}
			}
		}
	...
	public GenerateNewItemsetsTask(List<int[]> currentItemsets, int start, int end) {
		this.currentItemsets = currentItemsets;
		this.start = start;
		this.end = end;
	}
