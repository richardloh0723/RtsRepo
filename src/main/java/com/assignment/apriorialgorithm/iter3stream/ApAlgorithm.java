package com.assignment.apriorialgorithm.iter3stream;
...
//			minSup = (Double.valueOf(args[1]).doubleValue());
		...
		dataset = Files.readAllLines(new File(transaFile).toPath(), Charset.defaultCharset())
				.stream()
				...
				.map(stringArr -> {
					...
					for(int i = 0; i < stringArr.length; i++) {
						...
						if (intArray[i] + 1 > numItems)
							numItems = intArray[i] + 1;
					}
					numTransactions++;
					return intArray;
				})
