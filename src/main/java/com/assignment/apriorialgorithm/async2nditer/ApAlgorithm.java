package com.assignment.apriorialgorithm.async2nditer;
...
//		}
	...
	private void createNewItemsetsFromPreviousOnes() throws ExecutionException, InterruptedException {
		...
		Map<String, int[]> tempCandidates = new ConcurrentHashMap<>(); // temporary candidates
		// using async
		...
		for(int i = 0; i < numOfProcessors; i++) {
			...
			executors.execute(new
					GenerateNewItemsetsTask(currentItemsets,startEnd[0],startEnd[1], tempCandidates, controller));
		}
		controller.await();
...
class GenerateNewItemsetsTask implements Runnable {
	List<int[]> currentItemsets;
	int start;
	int end;
	Map<String, int[]> tempCandidates;
	CountDownLatch controller;
	...
	public GenerateNewItemsetsTask(List<int[]> currentItemsets, int start, int end,
								   Map<String, int[]> tempCandidates, CountDownLatch controller) {
		this.currentItemsets = currentItemsets;
		this.start = start;
		this.end = end;
		this.tempCandidates = tempCandidates;
		this.controller = controller;
	}
}
