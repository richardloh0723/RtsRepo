package com.assignment.kmeansalgorithm.parallelrunnableCHOSEN;/*
...
class Task implements Runnable {
	...
	public void run() {
		...
		for(int i = start; i <= end; i++) {
			double minimum = 999999.0;
			for(int j = 0; j < k; j++) {
				distance = com.assignment.kmeansalgorithm.parallelcallableandrunnable.DistanceSeq.eucledianDistance(centroids.get(j), features.get(i));
				if(distance < minimum) {
					minimum = distance;
					k1 = j;
				}
